...
//     try {
//       const response = await axios.get('https://firestore.googleapis.com/v1/projects/arunn-bank/databases/(default)/documents/customerAccounts');
...
//       await axios.patch(`https://firestore.googleapis.com/v1/projects/arunn-bank/databases/(default)/documents/customerAccounts/${requestId}`, {
...
//           }
//         });
//         await axios.post('https://firestore.googleapis.com/v1/projects/arunn-bank/databases/(default)/documents/Account', {
//           fields: {
//             accountNumber: { integerValue: accountNumber },
//             balance: { doubleValue: balance },
...
//       <h1 style={{ color: '#be1212', textAlign: 'center', marginBottom: '20px' }}>Approve Accounts</h1>
...
export const sendEmail = async (toEmail, templateId) => {
  try {
    const templateParams = {
      to_email: toEmail,
      firestore_api_link: 'https://firestore.googleapis.com/v1/projects/banking-system-3c21a/databases/(default)/documents/customerAccounts',
      // Add other template parameters here as needed
    };
 
    const serviceId = 'service_51939sg'; // Replace with your EmailJS service ID
    const userId = '9mB6usUxno8kIT9ts'; // Replace with your EmailJS user ID
...
function ApproveAccountsPage() {
  ...
  const fetchPendingRequests = async () => {
    try {
      const response = await axios.get('https://firestore.googleapis.com/v1/projects/banking-system-3c21a/databases/(default)/documents/customerAccounts');
      console.log('Response:', response);
  ...
  const updateRequestStatus = async (requestId, newStatus, email, phoneNumber) => {
    try {
      ...
      // Patch the request with updated fields
      await axios.patch(`https://firestore.googleapis.com/v1/projects/banking-system-3c21a/databases/(default)/documents/customerAccounts/${requestId}`, {
        fields: updatedFields
      });
 
      // Send email regardless of status
      const templateId = newStatus === 'approved' ? 'template_1c5x99g' : 'template_v1gnnkq'; // Use different template for rejection
      ...
      if (newStatus === 'approved') {
        ...
        await axios.post('https://firestore.googleapis.com/v1/projects/banking-system-3c21a/databases/(default)/documents/Account', {
          fields: {
                accountNumber: { integerValue: accountNumber },
                balance: { doubleValue: balance },
                name: { stringValue: existingRequest.fields.name.stringValue }, // Add account holder's name
  ...
  return (
    <div style={{ fontFamily: 'Arial, sans-serif',color:'white', maxWidth: '800px', margin: '0 auto', padding: '20px' }}>
      <h1 style={{ color: 'white', textAlign: 'center', marginBottom: '20px' }}>Approve Accounts</h1>
