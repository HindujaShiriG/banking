...
function ApplyLoan() {
    ...
    const checkAccountExists = async (accountNumber) => {
        try {
            const response = await fetch('https://firestore.googleapis.com/v1/projects/banking-system-3c21a/databases/(default)/documents/customerAccounts');
    ...
    const storeLoanData = async (loanData) => {
        try {
            const response = await fetch('https://firestore.googleapis.com/v1/projects/banking-system-3c21a/databases/(default)/documents/Loan', {
                method: 'POST',
                headers: {
